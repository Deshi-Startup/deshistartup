...
so the camera is retained. Regional polygons are submitted to the worker when the
geographic level changes or its detailed geometry arrives; measure colors and opacity use feature state, and selection
...
The national perimeter is dissolved from the same district coverage. Its muted
green outline stays legible at deeper zoom; selected and compared regions use stronger
...
- [geoBoundaries API record](https://www.geoboundaries.org/api/current/gbOpen/BGD/ADM2/): BGD-ADM2-16705992, boundary year **2020**, BBS/OCHA ROAP source, CC BY 3.0 IGO, source update 19 January 2023, build 12 December 2023; full source retrieved 14 September 2026.
- **Pinned input:** [full GeoJSON at release 9469f09](https://media.githubusercontent.com/media/wmgeolab/geoBoundaries/9469f09/releaseData/gbOpen/BGD/ADM2/geoBoundaries-BGD-ADM2.geojson), 46,554,183 bytes, verified against its Git LFS SHA-256. The full input stays outside Git and the client. `data/maps/boundaries.json` records its hash, retrieval date, licence, transformation versions and output hashes. The metadata links back to [HDX administrative boundaries](https://data.humdata.org/dataset/administrative-boundaries-of-bangladesh-as-of-2015); this older provenance is not evidence of a newly surveyed 2020 coastline.
...
- **Transformations:** simplify all 64 districts together with Shapely/GEOS coverage simplification in EPSG:3857, using Visvalingam tolerance 125 for overview and 15 for detail. These are area-derived cartographic scales, **not maximum errors or surveyed accuracy claims**. Reduce precision to a 0.00001-degree grid and verify valid, non-overlapping, exactly matching edges, retained polygon components and all district identifiers. Dissolve the same coverage into eight divisions and one national perimeter at each resolution: 73 analytical features per resolution, 72 statistical rows. The overview retains all 73. Detail is packaged separately for districts and divisions, each with the matching national perimeter split into styling components; this changes neither coordinates nor statistical geography. No snapping to basemap borders, roads or riverbanks. Existing statistical values and representative points are unchanged; symbols represent regions, not city centres or offices.
...
- **Context:** OpenFreeMap supplies optional detailed roads, water and deeper place labels. Its administrative boundary layers are omitted so a second dataset cannot compete with the analytical borders. Water/shorelines remain geographic context, not legal boundary evidence. Local analytical geometry remains available over the neutral background when hosted context fails.

### National perimeter and contextual fallback

The national perimeter uses the same district coverage, not a second boundary dataset. Its muted green line is 1.1px at zoom 5, 1.4px at zoom 7 and 1px at zoom 10; opacity settles from 0.7 at zoom 7 to 0.55 at zoom 10. Selected and compared regions retain their stronger solid and dashed outlines. All analytical fills and outlines share one MapLibre source, so the detail upgrade swaps every edge together. A 0.25-pixel tile-space rendering tolerance removes invisible detail at smaller scales while retaining close-up detail. Stored boundary coordinates remain unchanged. Do not disable this tolerance: unsimplified detailed national rings exceed the renderer’s 16-bit line-mesh limit and produce false diagonal borders. The boundary regression tests exercise the actual tiler for both resolutions and geographic levels, including cached detail at country scale.

Water and roads render above the analytical fills. Fill opacity decreases with zoom, district border widths range from 0.6 to 1.4px, and local labels use restrained weight. The neutral background is present from initialization. The overview payload is **2,282,862 bytes raw / 730,060 bytes gzip**. Once the URL is restored and Map is selected, its download starts alongside the lazy renderer; Data-only visits do not preload geometry. At zoom 9 or closer, a background request loads only the active geographic level: **6,814,459 bytes raw / 1,969,401 bytes gzip** for districts or **3,691,064 bytes raw / 1,068,703 bytes gzip** for divisions (Python gzip). Tests bound each payload. Bytes are cached as Blob URLs and parsed by MapLibre’s worker, avoiding application-side JSON parsing and outline reconstruction. Both levels share the same boundary source and atomic renderer update. Cached detail is reused for subsequent zooms and measure changes without moving the camera; switching geographic level below zoom 9 does not fetch unnecessary detail. Blob URLs are released on unmount. Camera bounds keep using the small overview rather than scanning detailed coordinates. Place labels retain their DOM elements between pans and omit off-screen regions. A failed detail request leaves the working overview in place with an explicit message and Retry; it never blanks the map. Upstream boundaries describe 2020 and are not surveyed land boundaries.
...
To update, verify each site's identity and locality against its authority, then check the specific linked OSM object and retain both source trails. Record publication/observation dates separately from retrieval. Do not imply real-time access, current vessel services, complete terminal coverage, or available industrial plots. Verify named-district joins without moving source coordinates to fit a boundary. Eight port positions lie inside their named overview district polygon. Bhomra’s OSM crossing-building centre is approximately **26 metres outside** the overview Satkhira polygon but inside its detailed polygon from the same 2020 source. Its original position is retained with a per-record note and a bounded 50-metre overview regression check; all nine are checked inside detailed districts. This does not establish legal border alignment.
