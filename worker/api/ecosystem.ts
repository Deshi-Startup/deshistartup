...
async function admitted(env: Environment, user: GoogleUser) {
  const owner = await contributorHash(user)
  const state = await moderationFor(getContributionBindings(env), owner)
  if (state.status !== 'active') return false
  return (await env.CONTRIBUTION_USER_RATE.limit({ key: owner })).success
}
...
export function createEcosystemHandler({ authenticate = requireUser, admit = admitted, now = () => new Date().toISOString() }: Dependencies = {}) {
  return async function ecosystem(request: Request, env: Environment): Promise<Response> {
    ...
    try {
      ...
      if (path === '/api/ecosystem/review' && request.method === 'GET') {
        ...
        const organizations = await db.prepare("SELECT o.id, o.slug, o.website, t.name FROM organizations o JOIN organization_text t ON t.organization_id = o.id AND t.locale = 'en' ORDER BY t.name LIMIT 1000").all()
        return json({ submissions: rows.results.map(row => ({ ...row, payload: JSON.parse(row.payload_json), payload_json: undefined })), organizations: organizations.results }, 200)
      }
...
export const handleEcosystem = createEcosystemHandler()
