#!/usr/bin/env node
// Local-only operator tools. No command in this file accesses a remote database or deploys.
...
const config = 'wrangler.ecosystem-local.jsonc'
const dbName = 'deshi-ecosystem-local'
...
function query(sql) {
  const data = JSON.parse(wrangler(['d1', 'execute', dbName, '--local', '--command', sql, '--json']))
  if (data.some(r => !r.success)) throw new Error('Local D1 query failed')
  return data[0]?.results || []
}
const action = process.argv[2]
if (action === 'init') {
  console.log(wrangler(['d1', 'migrations', 'apply', dbName, '--local']))
  ...
  if (!release || release.digest !== source.digest) throw new Error('The release does not match local D1.')
  ...
  console.log(`Marked ${requested} published in LOCAL D1. No remote deployment occurred.`)
} else {
  throw new Error('Usage: node scripts/ecosystem-local.mjs init | prepare | verify-build | publish <release-id>')
}
