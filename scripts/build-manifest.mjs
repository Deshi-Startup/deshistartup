...
fs.writeFileSync(path.join(root, 'public', 'page-published.json'), JSON.stringify(allPublished))
...
fs.writeFileSync(path.join(generatedDir, 'seo-pages.json'), JSON.stringify(seoPages, null, 1))
...
// use the custom domain, regardless of a deployment mirror's basePath.
{
  ...
  lines.push('')
  lines.push('Use the Bengali page as the source of truth when the two language versions differ. ' +
...
// excluded and receive noindex in the postbuild SEO pass.
{
  ...
  const written = seoPages.filter((page) => !page.stub).sort((a, b) => a.route.localeCompare(b.route))
  const writtenByKey = new Map(written.map((page) => [`${page.locale}:${page.slug}`, page]))
  const lines = [
  ...
  for (const page of written) {
    ...
    if (bnPage && enPage) {
      lines.push(`    <xhtml:link rel="alternate" hreflang="bn-BD" href="${escapeXml(canonicalUrl(bnPage.route))}" />`)
      lines.push(`    <xhtml:link rel="alternate" hreflang="en-BD" href="${escapeXml(canonicalUrl(enPage.route))}" />`)
      lines.push(`    <xhtml:link rel="alternate" hreflang="x-default" href="${escapeXml(canonicalUrl(bnPage.route))}" />`)
    }
    lines.push('  </url>')
  }
  lines.push('</urlset>')
...
// blocking them does not block the corresponding search/answer crawlers.
{
  ...
  console.log(`robots.txt: ${discoveryAgents.length} discovery and ${trainingAgents.length} training policies plus wildcard`)
}
...
// Consumed via https://img.shields.io/endpoint?url=<raw.githubusercontent URL to these files>.
{
  ...
  const color =
    pct >= 0.85 ? 'brightgreen' : pct >= 0.6 ? 'green' : pct >= 0.3 ? 'yellowgreen' : pct >= 0.1 ? 'orange' : 'red'
  const badge = (label, message) =>
    JSON.stringify({ schemaVersion: 1, label, message, color, cacheSeconds: 3600 }) + '\n'
  fs.writeFileSync(path.join(root, 'public', 'progress.json'), badge('লেখা শেষ', `${toBn(counts.written)}/${toBn(counts.pages)} পাতা`))
  fs.writeFileSync(path.join(root, 'public', 'progress.en.json'), badge('pages written', `${counts.written}/${counts.pages}`))
  console.log(`progress badges: ${counts.written}/${counts.pages} bn pages written`)
}
