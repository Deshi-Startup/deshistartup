...
- **Relative links between sibling guides (same section):** `next.config.mjs` has no `trailingSlash`, so routes render without a trailing slash (e.g. `/phase-one/e-tin-guide`, not `/phase-one/e-tin-guide/`). That means a plain `../other-slug` link resolves one level higher than intended (from `/phase-one/e-tin-guide` it resolves to `/other-slug`, dropping the section). Always write the section back into the relative path: `../phase-one/other-slug` (same literal string works for both the `(bn)` and `en` mirrors, since both trees are the same depth). Verify by checking the rendered `href` in `out/**/*.html` after a build if unsure.
...
- **Punctuation:** Never use an em dash (—) in user-facing content (page copy, titles, descriptions). The site's existing convention is an en dash (–) for a stylistic break (see e.g. `agents.md`'s own bullet points, `app/nav.config.js`); a comma, colon, or splitting into two sentences also works. Pick whichever reads most naturally, but never an em dash.
- **Writing a stub into a real guide:** Research the topic properly (web search, official portals, the Companies Act/NBR/RJSC as applicable) rather than relying on assumptions. Verify the stub's pre-listed sources are actually relevant to the topic before keeping them; drop irrelevant ones and add better ones. Before publishing, check `app/nav-groups.json` and sibling stub titles in the same section for topic overlap. If two stub slugs cover the same ground (e.g. a filing-deadlines table that duplicates another stub's planned scope), trim the new guide to its unique angle and cross-link to the other slug instead of duplicating content. When a topic doesn't have a matching slug yet, add it to the right group in `app/nav-groups.json` (see the Navigation rule above) so it doesn't fall into the generic "আরও গাইড / More guides" bucket. Run `npm run build` (which regenerates the manifest and Pagefind index) after writing a page to confirm it compiles and is grouped correctly.
