...
export default function CashTimingLab({ locale = 'bn' }: { locale?: 'bn' | 'en' }) {
  ...
  const x = (day: number) => 68 + day / 14 * 390
  ...
  return (
    <section className="cash-lab" aria-label={isEn ? "Cash timing simulation" : "ক্যাশ টাইমিং সিমুলেশন"}>
      <p className="cash-lab__intro"><strong>{isEn ? 'Fictional project, all amounts in BDT.' : 'এটি একটি কাল্পনিক প্রজেক্ট, সব এমাউন্ট টাকায়।'}</strong> {isEn ? 'A service is completed on day 1.' : '১ম দিনে একটি সার্ভিস সম্পন্ন হয়।'}
        {isEn ? ' Revenue is ' : ' রেভিনিউ '}{money(CASH_SCENARIO.revenue, isEn)}{isEn ? ', supplier cost is ' : ' টাকা, সাপ্লায়ার খরচ '}{money(CASH_SCENARIO.supplier, isEn)}{isEn ? ', and other costs are ' : ' টাকা, এবং অন্যান্য খরচ '}{money(CASH_SCENARIO.overhead, isEn)}{isEn ? '.' : ' টাকা।'}
        {isEn ? ' All costs belong to this project. Other costs are paid on day 5.' : ' সব খরচ এই প্রজেক্টের অন্তর্ভুক্ত। অন্যান্য খরচ ৫ম দিনে পেমেন্ট করা হয়।'}</p>
      <p>{isEn ? 'Before changing anything, predict: can a project earning BDT 10,000 cover its day-3 bill?' : 'কিছু পরিবর্তন করার আগে অনুমান করুন: ১০,০০০ টাকা লাভ করা একটি প্রজেক্ট কি তার ৩য় দিনের বিল কভার করতে পারবে?'}</p>
      <noscript><p>{isEn ? 'JavaScript is off. The worked result below remains readable. To compare by hand, move the receipt to day 2 and recalculate each balance.' : 'জাভাস্ক্রিপ্ট বন্ধ আছে। নিচের ফলাফলটি পড়া যাবে। নিজে হিসাব করে দেখতে, পেমেন্ট পাওয়ার দিন ২য় দিনে নিয়ে প্রতিটি ব্যালেন্স পুনরায় ক্যালকুলেট করুন।'}</p></noscript>
      ...
      <div role="status">
        {!result ? <p id={`${id}-error`}>{isEn ? 'Enter opening cash as a whole number from 0 to 1,000,000 BDT to see the result.' : 'ফলাফল দেখতে ওপেনিং ক্যাশ হিসেবে ০ থেকে ১,০০০,০০০ টাকার মধ্যে একটি পূর্ণসংখ্যা দিন।'}</p> : (
      ...
      {result && <>
        ...
        <figure className="cash-lab__chart">
          <svg viewBox="0 0 480 195" role="img" aria-label={isEn ? "Projected cash across 14 days" : "১৪ দিনে সম্ভাব্য ক্যাশ"} aria-describedby={`${id}-chart-desc`}>
            ...
            <line x1="68" x2="458" y1={y(0)} y2={y(0)} className="cash-lab__zero" />
            <text x="464" y={y(0) + 4}>{isEn ? '0' : '০'}</text>
            <text x="60" y={y(high) + 4} textAnchor="end">{money(high, isEn)}</text>
            {low < 0 && <text x="60" y={y(low) + 4} textAnchor="end">{money(low, isEn)}</text>}
            <text x="68" y="186">{isEn ? 'Start' : 'শুরু'}</text><text x="458" y="186" textAnchor="end">{isEn ? 'Day 14' : 'দিন ১৪'}</text>
        ...
        <div className="cash-lab__table">
          <table>
            <caption>{isEn ? 'Cash movements (BDT); days without movements are omitted.' : 'ক্যাশ মুভমেন্ট (টাকা); মুভমেন্ট ছাড়া দিনগুলো বাদ দেওয়া হয়েছে।'}</caption>
        ...
        <p>{isEn ? 'Each balance = previous balance + cash received − payments due. Day 14 includes every listed receipt and bill; it does not tell you whether earlier bills could be paid.' : 'প্রতিটি ব্যালেন্স = আগের ব্যালেন্স + প্রাপ্ত ক্যাশ − যে পেমেন্টগুলো করতে হবে। ১৪তম দিনে উল্লেখিত সব রিসিট এবং বিল অন্তর্ভুক্ত আছে; এটি নিশ্চিত করে না যে আগের বিলগুলো পেমেন্ট করা সম্ভব কিনা।'}</p>
      </>}
      <details className="cash-lab__assumptions">
        <summary>{isEn ? 'Read the assumptions before applying this lesson' : 'এই লেসনটি প্রয়োগ করার আগে এর পেছনের শর্তগুলো পড়ে নিন'}</summary>
        <p>{isEn ? 'This example assumes the full customer payment arrives on the selected day, before any same-day bills. Check clearing times in real life. It excludes taxes, interest, refunds, bad debts, and other business activity.' : 'এই উদাহরণে ধরে নেওয়া হয়েছে যে পুরো কাস্টমার পেমেন্ট নির্ধারিত দিনে, একই দিনের অন্য কোনো বিলের আগে আসবে। রিয়েল লাইফে ক্লিয়ারিং টাইম চেক করে নিবেন। এতে ট্যাক্স, ইন্টারেস্ট, রিফান্ড, ব্যাড ডেট এবং অন্যান্য বিজনেস অ্যাক্টিভিটি অন্তর্ভুক্ত নেই।'}</p>
        <p>{isEn ? 'Moving the supplier date assumes the supplier agrees beforehand. It is not permission to miss a due date. Additional cash needed describes the gap, not a recommendation to borrow.' : 'সাপ্লায়ারের পেমেন্ট ডেট পেছানোর মানে হলো সাপ্লায়ার আগে থেকেই এতে রাজি। এটি ডিউ ডেট মিস করার কোনো অনুমতি নয়। "অতিরিক্ত ক্যাশ প্রয়োজন" দিয়ে শুধুমাত্র গ্যাপ বোঝানো হয়েছে, এটি ধার নেওয়ার কোনো পরামর্শ নয়।'}</p>
      </details>
    </section>
  )
}
