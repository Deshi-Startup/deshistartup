...
export default function WorkingCapitalCalculator({ locale = 'en' }: { locale?: 'en' | 'bn' }) {
  ...
  const fields: { key: Key; label: string; hint: string; base?: Key }[] = [
    { key: 'creditSales', label: t('Monthly credit sales (BDT)', 'মাসে বাকিতে বিক্রি (টাকা)'), hint: t('Sales invoiced for later payment, not this month’s cash receipts.', 'যে বিক্রির টাকা পরে আসবে। এই মাসে হাতে পাওয়া টাকা নয়।') },
    ...
    { key: 'creditPurchases', label: t('Monthly credit purchases (BDT)', 'মাসে বাকিতে কেনাকাটা (টাকা)'), hint: t('Purchases on supplier credit, not all purchases or automatically COGS.', 'সাপ্লায়ারের কাছ থেকে বাকিতে কেনা। মোট কেনাকাটা বা বিক্রি হওয়া পণ্যের খরচ বসাবেন না।') },
    ...
    { key: 'collectionDays', label: t('Customer collection days', 'কাস্টমারের টাকা পেতে যত দিন'), hint: t('Average customer receivables ÷ (monthly credit sales ÷ 30).', 'কাস্টমারের কাছে গড় পাওনা ÷ (মাসে বাকিতে বিক্রি ÷ ৩০)।'), base: 'creditSales' },
    { key: 'paymentDays', label: t('Supplier payment days', 'সাপ্লায়ারকে টাকা দিতে যত দিন'), hint: t('Average supplier payables ÷ (monthly credit purchases ÷ 30).', 'সাপ্লায়ারের কাছে গড় দেনা ÷ (মাসে বাকিতে কেনাকাটা ÷ ৩০)।'), base: 'creditPurchases' }
  ]
  const invalid = (key: Key, base?: Key) => !Number.isFinite(values[key]) || values[key] < 0 || (base !== undefined && values[base] === 0 && values[key] !== 0)
  const rows = result ? [
    [t('Estimated inventory at cost (BDT)', 'আনুমানিক স্টক, খরচের হিসাবে (টাকা)'), result.inventory],
    [t('Estimated customer receivables (BDT)', 'কাস্টমারের কাছে আনুমানিক পাওনা (টাকা)'), result.receivables],
    [t('Less: estimated supplier payables (BDT)', 'বাদ: সাপ্লায়ারের কাছে আনুমানিক দেনা (টাকা)'), result.payables],
    [t('Operating cash tied up (BDT)', 'এই হিসাবে আটকে থাকা টাকা'), result.tiedUp],
    ...
    <div aria-live="polite" aria-atomic="true">
      {!result ? <p className="calc__verdict" id={`${id}-error`}>{t('Enter a finite number of zero or more in every field. If a monthly amount is zero, set its matching days to zero. If the numbers are too large to calculate, reduce them. No estimate is shown until the inputs are valid.', 'প্রতিটি ঘরে শূন্য বা তার চেয়ে বড় সংখ্যা দিন। মাসের অঙ্ক শূন্য হলে তার সঙ্গে মেলা দিনের ঘরেও শূন্য দিন। সংখ্যা এত বড় হলে যে হিসাব করা যাচ্ছে না, তা কমিয়ে দিন। ঘরগুলো ঠিক না হওয়া পর্যন্ত ফল দেখানো হবে না।')}</p> : <>
        ...
        <p className="calc__verdict">{result.tiedUp < 0
          ? t('Supplier payables exceed the modeled stock and receivables. This is not spare cash: supplier bills still need paying.', 'এই হিসাবে স্টক ও পাওনার চেয়ে সাপ্লায়ারের দেনা বেশি। টাকাটা বাড়তি ক্যাশ নয়, সাপ্লায়ারের বিল তো দিতেই হবে।')
          : result.tiedUp === 0
            ? t('These modeled balances net to zero. This does not mean the business needs no cash.', 'এই হিসাবে দেনা বাদ দিয়ে শূন্য থাকে। তার মানে ব্যবসা চালাতে আর ক্যাশ লাগবে না, এমন নয়।')
            : t('This estimates stock and unpaid invoices, less supplier credit. It is not your bank balance or total funding requirement.', 'স্টক ও কাস্টমারের পাওনা থেকে সাপ্লায়ারের দেনা বাদ দেওয়া হয়েছে। ব্যাংকে কত টাকা আছে বা মোট কত ফান্ডিং লাগবে, এটা সেই হিসাব নয়।')}</p>
        {result.cycle < 0 && <p>{t('The cycle is negative: the modeled collection cycle ends before suppliers are paid. This does not guarantee a negative taka balance or cash available to spend.', 'সাইকেল ঋণাত্মক। এই হিসাবে সাপ্লায়ারকে দেওয়ার আগেই বিক্রির টাকা আসে। তবে টাকার ফলও ঋণাত্মক হবে বা হাতে খরচ করার ক্যাশ থাকবে, এমন নিশ্চয়তা নেই।')}</p>}
        {(values.creditSales === 0 || values.cogs === 0 || values.creditPurchases === 0) && <p>{t('A zero monthly amount contributes no balance here. Old stock, unpaid invoices and existing supplier bills still need a separate cash schedule.', 'মাসের অঙ্ক শূন্য হলে এই হিসাবে সেই খাতে কোনো টাকা ধরা হয় না। পুরোনো স্টক, বকেয়া ইনভয়েস ও সাপ্লায়ারের বিলের জন্য আলাদা করে টাকা আসা-যাওয়ার তারিখ লিখে রাখুন।')}</p>}
      </>}
    </div>
