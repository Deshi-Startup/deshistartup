...
export default function Ideas({ locale }: { locale: Locale }) {
  const summaries: IdeaSummary[] = ecosystem.approaches.map(idea => {
    ...
    return {
      ...
      title: idea[locale].title, summary: idea[locale].summary,
      search: [idea.en.title, idea.bn.title, idea.en.summary, idea.bn.summary, problem.en.title, problem.bn.title, ...Object.values(sectors[problem.sector as Sector]), ...Object.values(kinds[idea.kind]), ...problem.places.flatMap(place => Object.values(places[place as Place]))].join(' ')
    }
  })
