    ...
    <div hidden={!authorized} className="submission-history">
      ...
      {rows.map(item => 'kind' in item.payload && <details key={item.id} ref={selected?.id === item.id ? selectedRecord : undefined} open={selected?.id === item.id ? true : undefined} className="submission-record">
        ...
        <div className="submission-record-body">
          {item.decision_note && <div className="submission-note"><h3>{t('Reviewer’s note', 'পর্যালোচকের মন্তব্য')}</h3><p>{item.decision_note}</p></div>}
          {item.published && item.idea_id ? <a className="ideas-inline-link" href={ideaPath(locale, ideaSlug(item.idea_id))}>{t('View published idea', 'প্রকাশিত আইডিয়া দেখুন')}</a> : item.idea_id ? <p className="ideas-field-help">{t('The linked idea is currently unpublished.', 'লিংক করা আইডিয়াটি এখন প্রকাশিত নেই।')}</p> : item.status === 'approved' && <p>{t('We’ll prepare the idea in English and Bangla before publication.', 'প্রকাশের আগে আইডিয়াটি বাংলা ও ইংরেজিতে গুছিয়ে নেব।')}</p>}
