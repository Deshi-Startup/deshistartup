...
export default function IdeaDraft({ locale }: { locale: Locale }) {
  ...
  return <div className="ideas-draft-page">
    ...
    {submitted ? <section className="submission-confirmation" aria-labelledby="submission-confirmed"><h1 ref={confirmation} tabIndex={-1} id="submission-confirmed">{t('Idea submitted.', 'আইডিয়া জমা হয়েছে।')}</h1><p>{t('Our editors will review it. You’ll find their decision and note in Your submissions.', 'সম্পাদকীয় দল আইডিয়াটি দেখবে। সিদ্ধান্ত ও কারণ জানতে জমা দেওয়া আইডিয়ার পাতা দেখুন।')}</p><div className="ideas-draft-bottom"><a className="ideas-button" href={submissionPath(locale, submitted)}>{t('View submission', 'জমা দেওয়া আইডিয়া দেখুন')}</a><button className="ideas-text-button" type="button" onClick={() => { setSubmitted(''); setDraft(emptyDraft); setMessage(''); retry.current = null; requestAnimationFrame(() => titleInput.current?.focus()) }}>{t('Add another idea', 'আরেকটি আইডিয়া দিন')}</button></div></section> : <>
