...
export default function ConnectionForm({ locale, companies, problems }: { locale: Locale; companies: Company[]; problems: { id: string; slug: string; title: string }[] }) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (!raw?.work && saved.problemId) {
        const legacy = JSON.parse(localStorage.getItem(`deshi-startup:ideas:startup-draft:v1:${saved.problemId}`) || '{}')
  ...
  return <>
    ...
    <form className="ideas-draft-form connection-form" onSubmit={submit}><fieldset disabled={!ready || busy}><legend className="sr-only">{t('Company connection', 'কোম্পানির কাজের তথ্য')}</legend>
      ...
      <div className="connection-company"><h2>{t('Company', 'কোম্পানি')}</h2>{selected ? <div className="company-selected"><div><strong>{selected.name}</strong><p>{domain(selected.website)}</p></div><button className="ideas-text-button" type="button" onClick={() => { edit({ organizationId: '' }); setNewCompany(false) }}>{t('Change', 'বদলান')}</button></div> : <>
        {!newCompany && <><label className="sr-only" htmlFor="connection-company-search">{t('Find an existing company', 'আগের তালিকায় কোম্পানি খুঁজুন')}</label><input id="connection-company-search" type="search" value={search} maxLength={120} onChange={e => setSearch(e.target.value)} placeholder={t('Search by name or website…', 'নাম বা ওয়েবসাইট লিখে খুঁজুন…')} /><ul className="company-matches">{matches.map(c => <li key={c.id}><button type="button" onClick={() => { edit({ organizationId: c.id }); setSearch('') }}><strong>{c.name}</strong><span>{domain(c.website)}</span><small>{c.description}</small></button></li>)}</ul>{search.trim() && !matches.length && <p className="ideas-field-help">{t('No matching company. You can add it below.', 'প্রকাশিত প্রোফাইলে মিল পাওয়া যায়নি।')}</p>}<button type="button" className="ideas-text-button" onClick={() => setNewCompany(true)}><IdeaIcon name="plus" />{t('Add a new company', 'নতুন কোম্পানির তথ্য দিন')}</button></>}
        {newCompany && <div className="connection-new"><button type="button" className="ideas-text-button" onClick={() => setNewCompany(false)}>{t('Search existing companies instead', 'আগের তালিকায় খুঁজুন')}</button><label htmlFor="connection-name">{t('Company name', 'কোম্পানির নাম')}</label><input id="connection-name" required maxLength={100} value={draft.name} onChange={e => edit({ name: e.target.value })} /><label htmlFor="connection-website">{t('Website or public product page', 'ওয়েবসাইট বা পণ্যের প্রকাশ্য পাতা')}</label><input id="connection-website" type="url" required maxLength={500} placeholder="https://" value={draft.website} onChange={e => edit({ website: e.target.value })} /><label htmlFor="connection-description">{t('What does the company do?', 'কোম্পানি কী করে?')}</label><textarea id="connection-description" required minLength={20} maxLength={500} rows={3} value={draft.description} onChange={e => edit({ description: e.target.value })} /><label htmlFor="connection-role">{t('Type', 'ধরন')}</label><select id="connection-role" value={draft.role} onChange={e => edit({ role: e.target.value })}>{Object.entries(organizationRoles).map(([id, label]) => <option key={id} value={id}>{label[locale]}</option>)}</select></div>}
      </>}</div>
      <label htmlFor="connection-work">{t('How is it working on this problem?', 'এই সমস্যা নিয়ে কী কাজ করছে?')}</label><textarea id="connection-work" required minLength={20} maxLength={2000} rows={4} value={draft.work} onChange={e => edit({ work: e.target.value })} />
