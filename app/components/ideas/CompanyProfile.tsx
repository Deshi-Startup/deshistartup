...
export default function CompanyProfile({ locale, id }: { locale: Locale; id: string }) {
  ...
  if(profile?.contact && (profile.contact.address || profile.contact.email || profile.contact.phone)) { const contact=profile.contact; add('contact',en?'Office & contact':'অফিস ও যোগাযোগ',<>{contact.address&&<p>{contact.address[locale]}</p>}{(contact.email||contact.phone)&&<div className="companies-actions">{contact.email&&<a href={`mailto:${contact.email}`}>{contact.email}<IdeaIcon name="arrow"/></a>}{contact.phone&&<a href={`tel:${contact.phone.replace(/[^+0-9]/g,'')}`}>{en?'Phone: ':'ফোন: '}{digits(contact.phone)}</a>}</div>}{citations(contact.sources)}</>) }
